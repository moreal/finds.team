...
internal class JooqCommandRequestStore(
  ...
  override fun reserve(request: CommandRequest): CommandReservation = scope.access {
    ...
    if (inserted == 1) {
      ...
      // ON CONFLICT waits on an uncommitted competing insert. A new READ COMMITTED statement
      // sees the winner; at stricter isolation, serialization failures propagate without retry.
      val row = checkNotNull(db.selectFrom(COMMAND_REQUESTS).where(keyCondition(request.key)).forUpdate().fetchOne()) {
        "Conflicting command reservation disappeared"
      }
      if (row.requestHash != request.requestHash.value) CommandReservation.Conflict
