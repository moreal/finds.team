...
data class FindsProperties(
  ...
  data class Source(
    ...
    val retryDelays: List<Duration> = listOf(Duration.ofSeconds(1), Duration.ofSeconds(3)),
    val robotsSuccessTtl: Duration = Duration.ofHours(1),
    val robotsFailureTtl: Duration = Duration.ofMinutes(5),
    @field:Min(0) val sitemapMaximumDepth: Int = 4,
    ...
    init {
      ...
      require(retryDelays.all { !it.isNegative && !it.isZero }) { "retryDelays must be positive" }
  ...
  data class Crawl(
    ...
    val failureRetryInterval: Duration = Duration.ofMinutes(30),
    ...
    init {
      ...
      requirePositive(failureRetryInterval, "failureRetryInterval")
